...
function asElements(graph: GraphData) {
  return [
...
export function GraphView() {
  ...
  const fetchGraph = async (): Promise<GraphData> => {
    ...
    const transactionRes = await fetch(`${API_URL}/transactions?${params.toString()}`);
    const txGraph = (await transactionRes.json()) as GraphData;

    if (txGraph.nodes.length > 0) {
      return txGraph;
    }

    const searchRes = await fetch(`${API_URL}/search?limit=1`);
    const searchData = (await searchRes.json()) as { nodes: Array<{ id: string }> };
    const seedId = searchData.nodes[0]?.id;

    if (!seedId) {
      return { nodes: [], edges: [] };
    }

    const entityRes = await fetch(`${API_URL}/entity/${seedId}?depth=2&limit=500`);
    return entityRes.json() as Promise<GraphData>;
  };
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      cy.on('tap', 'node', async (event) => {
        ...
        const resp = await fetch(`${API_URL}/entity/${id}?depth=1&limit=300`);
      ...
      if (graph.nodes.length === 0) {
        setStatus('Sem dados. Rode: npm run generate e reinicie a API.');
      } else {
